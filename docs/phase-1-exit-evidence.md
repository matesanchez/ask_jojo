...
- **Publicdrive first walk:** the initial `P:\` walk is long-running (observed multi-hour; exact duration TBD once the in-flight run finishes). Incremental re-runs after the first walk land inside the hour budget because the manifest short-circuits already-ingested files. We are tracking two follow-ups for resilience (task #58): (a) streaming writes during the walker's enumeration phase so progress is visible and crash recovery is cheap; (b) per-connector singleton lock so a second invocation of `sync publicdrive` can't race the first and corrupt the manifest.
  ...
  **Observational note (2026-04-23, later in day).** A manual publicdrive invocation was left running from ~10:39 PT. At the 2-hour mark (12:40 PT-ish) the process had consumed ~45 min of CPU time (PID 14320, WS ~2.3 GB) but `ask_jojo_raw/publicdrive/` did not exist yet and the manifest was still at its scaffolding state — classic FU-1 symptom: walker alive and working, driver buffering all writes until end-of-run, zero visibility into how close it was to completion. We killed the process rather than wait indefinitely, on the reasoning that (a) Phase 1 exit was already satisfied by OneDrive + SharePoint alone, and (b) `JojoBot-PublicDrive` daily scheduled task will catch up incrementally across the 7-day soak. Preserved audit trail lives in `ask_jojo_raw/_changes/2026-04-23.json` (~2.5 MB of aggregated change records from today's runs). If publicdrive still shows zero output by 2026-04-30, treat that as a real bug and dig into FU-1 urgently — candidate next step is passing a narrower `--source` rather than `P:\` root, e.g., one Protein Sciences subtree at a time.

  **Observational note (2026-04-24, morning — Day 1 of soak, stress-test attempt).** Last night we attempted an overnight manual publicdrive walk (2026-04-23 15:52 kickoff with `standby-timeout-ac 0` and `hibernate-timeout-ac 0` set, three other scheduled tasks disabled to avoid manifest-race with the in-flight walker). At 2026-04-24 ~09:00 the python process was still "alive" (PID 18136) but had consumed only **0.03 seconds of CPU time in 17 hours**, with a 12 MB working set and a log `LastWriteTime` frozen at 15:55:18. Zero `.md` files landed in `ask_jojo_raw/publicdrive/`; zero manifest entries for publicdrive. Diagnosis: the `os.scandir`/`os.stat` syscall went kernel-blocked when Windows NIC selective-suspend tore down the SMB session overnight (we disabled system sleep but not the NIC's "allow turn off" checkbox). `P:\` was reachable again by morning but the orphaned handle never recovered. Killed PID 18136, re-enabled scheduled tasks.

  We are treating 2026-04-23 as **Day 1 of the soak** (stress-test attempt) rather than sliding the window — the failure itself is soak-evidence of a real bug we now understand (filed as FU-9: per-SMB-call watchdog timeout in `_walk`). Day 1 produced no scheduled-task runs for `drive` / `onedrive` / `publicdrive` (tasks were disabled) and two SharePoint runs (11:07 with a fresh token and 15:00 with an expired one, both logged cleanly). Day 2 (2026-04-24 02:00 → 2026-04-25 02:00) is the first fully-unattended cadence window. Soak window remains 2026-04-23 → 2026-04-30 inclusive.
