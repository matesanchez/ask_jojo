...
- **"Make Unicorn-themed slides for our team"** — contains `unicorn`. Regex routes `v1` (false positive). Operator override: `wiki` (or whatever Phase 5 produces for "make slides"). Reasoning: in everyday English, `unicorn` is common (mythical, "startup unicorn", UI brand), but in a Nurix Q&A context the word almost always means UNICORN-the-software, so the regex's bias is correct most of the time. The Haiku classifier on API day handles the disambiguation by reading surrounding context. Until then, this is a known regex limitation: `\bunicorn\b` alone is a coarse signal. Documented here rather than added to the wiki-override list because over-tightening the regex would mask real UNICORN-software questions.
- **General router guidance for Phase 4 reviewers:** if a question routes the wrong way and the operator has to override, file the misroute in this list with a one-line reason. The patterns accumulate into the Haiku classifier's training data on API day. Don't tighten the regex on a single misroute; the regex is a coarse safety net, not a precision tool.
