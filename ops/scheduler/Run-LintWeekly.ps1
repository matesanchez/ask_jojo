# Run-LintWeekly.ps1
#
# Phase 6 weekly lint run. Mirrors Run-GraphifyRebuild.ps1's pattern:
# tees stdout+stderr to a dated log file, mirrors success/failure to the
# Windows Application event log under source 'JojoBot'.
#
# The weekly task runs at 03:00 local time on Sundays.
#
# Usage:
#   Run-LintWeekly.ps1 [-WikiRoot PATH] [-LogRoot PATH] [-MaxRuntimeMinutes N]
#
...
$LogDir = Join-Path $LogRoot "lint-weekly"
if (-not (Test-Path $LogDir)) {
    New-Item -ItemType Directory -Path $LogDir -Force | Out-Null
}

$LogFile = Join-Path $LogDir ("lint-weekly-" + (Get-Date -Format 'yyyy-MM-dd') + ".log")
...
if (-not $LintExe) {
    ...
    Write-EventSafe -Message $msg -EventId 7203 -EntryType Error
    exit 1
}

Write-LogLine ("[{0}] lint-weekly starting" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'))
...
try {
    $cmdArgs = @('weekly', '--wiki', $WikiRoot)
    $output = & $LintExe @cmdArgs 2>&1 | Out-String
    Write-LogLine $output

    $duration = (Get-Date) - $startTime
    $msg = ("lint-weekly completed in {0:F1} minutes" -f $duration.TotalMinutes)
    Write-LogLine ("[{0}] INFO {1}" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $msg)
    Write-EventSafe -Message $msg -EventId 7202 -EntryType Information
    exit 0
} catch {
    $duration = (Get-Date) - $startTime
    $msg = ("lint-weekly FAILED after {0:F1} minutes: {1}" -f $duration.TotalMinutes, $_.Exception.Message)
    Write-LogLine ("[{0}] ERROR {1}" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $msg)
    Write-EventSafe -Message $msg -EventId 7203 -EntryType Error
    exit 1
}
